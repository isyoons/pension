...
public class MySqlGalleryDao implements GalleryDao {
	...
	public int updateCount(int no) throws Exception {
		SqlSession sqlSession = sqlSessionFactory.openSession();
		try {
			int result = sqlSession.update("kr.pe.temp.pension.dao.GalleryDao.updateCount", no);
			if(result > 0) sqlSession.commit();
			return result;
		} finally {
			sqlSession.close();
	...
	public int selectTotalCount() throws Exception {
		SqlSession sqlSession = sqlSessionFactory.openSession();
		try {
			return sqlSession.selectOne("kr.pe.temp.pension.dao.GalleryDao.selectTotalCount");
		} finally {
			sqlSession.close();
		}
